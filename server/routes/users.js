...
router.post("/register",function(req,res) {
  Register.register(req.body,connection,res)
})
router.get("/register/maxid",function(req,res) {
  Search.getResterid(connection,res)
})
...
//新增用户收货订单
router.post("/receiverorder",(req,res)=> {
  Add.add("receivergoods",req.body,connection,res)
})

//获取用户收货订单所有信息（地址、购物车图片等）
router.get("/receivergoods/all", (req, res) => {
  Search.getAllReceiverMsg(req.query.id, connection, res)
})
// 查找用户收货订单
router.get("/receivergoods",(req,res) => {
  Search.getSearch(req.query,"receivergoods",connection,res);
})
//取消用户订单
router.delete("/delOrder",(req,res) => {
  Delete.deleteOrder(req.body,connection,res);
})
