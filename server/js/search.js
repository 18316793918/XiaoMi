...
function getResterid(connection,res){
    let sql = 'select max(userid) from user'
    ExecuteSQL.execute(sql, connection, res);
}
...
function getAllProvince(connection, res) {
    let sql = `select province from provinces`
    ExecuteSQL.execute(sql, connection, res);
}

function getAllCity(body, connection, res) {
    let sql = `select city,cityid from cities as c,provinces as p where province = '${body.province}' and p.provinceid = c.provinceid`
    ExecuteSQL.execute(sql, connection, res);
}

function getAllCounty(body, connection, res) {
    let sql = `select area,areaid from areas as a,cities as c where  city = '${body.city}' and a.cityid = c.cityid;`
    ExecuteSQL.execute(sql, connection, res);
}
...
//获取用户收货订单的所有信息
function getAllReceiverMsg(id, connection, res) {
    let sql = `select r.*,u.region,u.address,u.phone,u.username
      from receivergoods as r,useraddress as u where r.userid = ${id} and r.addressid = u.id order by receiverid desc`;
    ExecuteSQL.execute(sql, connection, res);
}
module.exports = {
    getSearch,
